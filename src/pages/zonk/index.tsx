...
export default function Zonk() {
  ...
  }, [newSelectedBot, currentIndexSelected]);
  ...
  const botLogical = (): number[] => {
    ...
    let powerSet: number[][] = setPowerset([
      ...new Array(counterDices[1] < 3 ? counterDices[1] : 0).fill(1),
      ...new Array(counterDices[5] < 3 ? counterDices[5] : 0).fill(5),
    ]);
    ...
    for (let powS of powerSet) {
      ...
      for (let comb of combinations) {
        ...
        let newComb: diceForBot = {
          score: newScore,
          dices: [...combByPower.dices, ...comb.dices],
          powerWay: newScore - (powS.length + comb.dices.length) * weights.kDice,
        };
  ...
  const moveBot = () => {
    ...
    let allDices: number[] = Object.values(botDices);
    let out: number[] = answer.map((item) => {
      let ind = allDices.indexOf(item);
      allDices[ind] = -1;
      return ind;
    });
    ...
    return Array.from({ length: amount }, (_, key) => {
      return (
        <div
          ...
          key={key}
          className={`dice ${key === hoverDice ? "dice--active" : null} ${selected.includes(key) ? "dice--selected" : null}`}
        >
          {Array.from({ length: dices[key + 1] }, (_, keyPin) => {
            return <div key={`${key}_${keyPin}`} className="pin"></div>;
          })}
        </div>
      );
    });
  ...
  return (
    <main className="main">
      ...
      <div className="rowDices">{RowDices(selectDicesBot, botDices, userMove ? 0 : amountDices)}</div>
      ...
      <div style={{ display: "flex" }}>
        ...
        <div className="control">
          <p>
            Отложить кость<span>E</span>
          </p>
          <p>
            Записать счет и бросить снова<span>F</span>
          </p>
          <p>
            Записать счет и передача хода<span>Й</span>
          </p>
          <p>
            Выбор кости
            <span>
      ...
      <div className="rowDices">{RowDices(selectDicesUser, userDices, userMove ? amountDices : 0)}</div>
    </main>
  );
}
